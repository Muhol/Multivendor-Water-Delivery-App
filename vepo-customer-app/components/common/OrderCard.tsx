...
const OrderCard = ({ order }: Props) => {
  ...
  return (
    <Animated.View className=" flex-1 gap-2 bg-white rounded-xl p-4 transition-all duration-300">
      {/* DATE  AND AMOUNT */}
      <View className="flex-row justify-between items-center gap-[50px]">
        <ComicText text={`Date: ${Date}`} style={""} />
        <ComicText text={`Total Amt: Ksh ${1000}`} style={"text-lg"} />
      </View>
      ...
      <View className="flex-row justify-between gap-5  ">
        ...
        <View className="gap-2 flex-row items-center">
          ...
          <ComicText text={`${order}`} style="capitalize" />
        </View>
        {/* Customer actions */}
        {action != "" &&
          <TouchableOpacity
          ...
          >
            <View className="py-2 px-3 bg-accentbg rounded-xl">
              <ComicText text={action} style="" />
            </View>
          </TouchableOpacity>
      ...
      {order ==="completed" && 
        <TouchableOpacity 
          ...
          activeOpacity={0.7}
        >
          <View className="py-2 px-3 bg-accentbg rounded-xl items-center">
              <Text className="font-semibold text-lg">Re-Order</Text>
          </View>
        </TouchableOpacity>
      }
      ...
      <View className="gap-1 bg-accentbg/5 rounded-lg p-2">
        <Text className="font-semibold text-lg">Vendor Details </Text>

        {/* PHONE NUMBER */}
        <View className="flex-row gap-1 items-center">
          <Text className="font-semibold text-lg">Phone: </Text>
          <ComicText text={`${"0712342554"}`} />
        </View>
      ...
      {showItems &&
        orders.map((item, index) => {
          return (
            <View
              key={index}
              className="p-2 bg-accentb flex-row gap-2 flex-1 rounded-xl "
            >
              {/* IMAGE  */}
              ...
              <View className="flex-1 gap-2 justify-center ">
                ...
                <View className="flex-row gap-5 justify-between items-end">
                  <View className="">
                    <ComicText text={`Qty: ${3}`} style={"text-lg"} />
                    <ComicText text={`Price: Ksh ${100}`} style={""} />
                  </View>
                  <View className="items-end">
                    <ComicText text={"Subtotal"} style={"text-lg"} />
                    <ComicText text={` Ksh ${300}`} style={""} />
                  </View>
                </View>
              </View>
            </View>
        ...
        <>
          ...
          <View className="gap-1">
            ...
            <View className="flex-row gap-2 items-end">
              <Text className="font-semibold text ">Delivery fee:</Text>
              <ComicText text={`Ksh ${200}`} style={""} />
            </View>
            {/* Location */}
            ...
            {/* total amount */}
            <View className="flex-row gap-2 items-end">
              <Text className="font-semibold text ">Total Amt:</Text>
              <ComicText text={`Ksh ${200}`} style={""} />
            </View>
          </View>
        </>
      ...
      {!showItems ? (
        <>
          <View className="">
            <ComicText text={`${orders.length} items`} />
          </View>
          <TouchableOpacity
            className=""
            activeOpacity={0.8}
